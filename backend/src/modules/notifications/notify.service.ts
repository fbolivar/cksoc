...
function buildMessages(rule: AlertRule, matchedCount: number) {
  ...
  const subject = `🚨 HexWatch · ${rule.name}`;
  const lines = [
    `*🚨 ALERTA DE SEGURIDAD · HexWatch*`,
    ``,
    `*Regla:* ${rule.name}`,
  ...
  <div style="font-family:Inter,Arial,sans-serif;max-width:560px;margin:auto;background:#0f1613;color:#e6f2ec;border-radius:12px;overflow:hidden;border:1px solid #1d2b25">
    <div style="background:#0f3d24;padding:18px 24px;border-bottom:3px solid #85b425">
      <h2 style="margin:0;font-size:16px;color:#fff">Centro de Operaciones de Seguridad · HexWatch</h2>
    </div>
    <div style="padding:24px">
      ...
      <p style="color:#6b7c74;font-size:12px;margin-top:20px">${now} · HexWatch</p>
    </div>
  </div>`;
...
export async function testSend(channel: 'email' | 'telegram', target: string): Promise<void> {
  const subject = '✅ HexWatch · Mensaje de prueba';
  const text = 'Mensaje de prueba del Centro de Operaciones de Seguridad de HexWatch. Si lo recibes, el canal funciona correctamente.';
  if (channel === 'email') {
    await sendEmail(
      [target],
      subject,
    ...
    await sendTelegram([target], `*✅ HexWatch*\n${text}`);
