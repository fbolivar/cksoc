/**
 * Checks deterministas de postura del SonicWall contra mejores prácticas de la
 * industria (alineados a hardening SonicWall / hardening de SonicWall). Salen de datos
...
export function runChecks(s: FwSnapshot): Check[] {
  ...
  c.push({
    ...
    remediacion: 'Habilitar OTP/2FA por administrador en SonicOS: Device > Users > Local Users & Groups → editar el usuario admin → activar One-Time Password (TOTP/email).',
    referencia: 'hardening SonicWall 1.x · Hardening de administración',
  });
  ...
  c.push({
    ...
    remediacion: 'Limitar el origen de la gestión: en SonicOS restringir el login admin por IP/red confiable (Device > Users, y Access Rules que solo permitan HTTPS/SSH de gestión desde las redes de confianza).',
    referencia: 'hardening SonicWall · Restringir gestión a redes confiables',
  });
  ...
  c.push({
    ...
    remediacion: 'Deshabilitar la gestión (HTTPS/SSH/Ping) en la interfaz WAN (X1): Network > Interfaces → editar WAN → sección Management; administrar solo por LAN o VPN.',
    referencia: 'hardening SonicWall · Sin gestión en interfaces no confiables',
  });
  ...
  c.push({
    ...
    remediacion: 'Deshabilitar Telnet/HTTP de gestión y dejar solo HTTPS/SSH: Device > Settings > Administration.',
    referencia: 'hardening SonicWall · Deshabilitar protocolos inseguros',
  });
  ...
  c.push({
    ...
    remediacion: 'Fijar el cierre de sesión por inactividad ≤ 15 min: Device > Settings > Administration > Log out after inactivity.',
    referencia: 'hardening SonicWall · Expiración de sesiones inactivas',
  });
  ...
  c.push({
    ...
    remediacion: 'Habilitar bloqueo tras intentos fallidos de login admin: Device > Settings > Administration > Login Security.',
    referencia: 'hardening SonicWall · Protección contra fuerza bruta',
  });
  ...
  c.push({
    ...
    remediacion: 'Forzar TLS fuerte para la gestión y deshabilitar cifrados/protocolos débiles: Device > Settings > Administration.',
    referencia: 'hardening SonicWall · Cifrado de servicios de gestión',
  });
  ...
  c.push({
    ...
    referencia: 'hardening SonicWall · Reglas de firewall específicas',
  });
  ...
  c.push({
    ...
    remediacion: 'Aplicar Servicios de Seguridad (Gateway AV, IPS, Anti-Spyware, Content Filter, App Control) e inspección DPI-SSL en las reglas que lo ameriten: Policy > Rules and Policies + Security Services.',
    referencia: 'hardening SonicWall · Inspección de amenazas',
  });
  ...
  c.push({
    ...
    remediacion: 'Habilitar el registro en las reglas relevantes y el envío de syslog al SIEM: Policy > Rules and Policies (Logging) + Device > Log > Syslog.',
    referencia: 'hardening SonicWall · Registro y monitoreo',
  });
  ...
  c.push({
    ...
    referencia: 'hardening SonicWall · SNMP seguro',
  });
